...
public class BotHeadArm : MonoBehaviour
{
    Rigidbody2D rb;
    ...
    void OnTriggerEnter2D(Collider2D col) {
        ...
        if (col.tag == "Torso") {
            Destroy(col.gameObject);
            gameController.PickedUpTorsoFromHeadArm();
        }

        if (col.tag == "Arm") {
            Destroy(col.gameObject);
            gameController.PickedUpArmFromHead();
        }
